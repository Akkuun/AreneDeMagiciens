...
func _tick(_delta: float) -> Status:
	...
	# récupère la navigation map
	var navigation_map: RID = agent.get_world_3d().get_navigation_map()
	
	# vérifi que la navigation map est synchronisée (évite l'erreur au premier tick)
	if NavigationServer3D.map_get_iteration_id(navigation_map) == 0:
		return RUNNING  # Attend la première synchronisation
